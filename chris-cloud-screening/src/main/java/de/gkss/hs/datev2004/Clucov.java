...
public class Clucov {
    ...
    Logger logger;
    Random rgen;
    public HashMap<Short, Cluster> clusterMap;
    int iteration;
    short next_cluster;
    ...
    public Clucov(DataSet ds, Logger logger) {
        this(ds, logger, new Random(5489));
    }
    ...
    Cluster[] alive() {
        Cluster[] res = new Cluster[clusterMap.size()];
        Iterator<Short> keys = this.clusterMap.keySet().iterator();
        int i = 0;
        while (keys.hasNext()) {
            res[i] = this.clusterMap.get(keys.next());
            i++;
        }
        return res;
    }
    ...
    void calculate_moments() {
        Cluster cl;
        for (short s : this.clusterMap.keySet()) {
            cl = this.clusterMap.get(s);
        ...
        for (int np = 0; np < this.ds.npoints; np++) {
            ...
            cl = this.clusterMap.get(this.ds.group[np]);
    ...
    public boolean clustering(boolean last) {
        ...
        for (Short s : this.clusterMap.keySet()) {
            Cluster rc = this.clusterMap.get(s);
    ...
    boolean combine() {
        ...
        while (change) {
            ...
            if (thigh > this.t_comb) {
                ...
                this.clusterMap.put(nn, both);
                this.clusterMap.remove(clusters[cmax].group);
                this.clusterMap.remove(clusters[dmax].group);
                res = true;
            } else {
                change = false;
            }
...
//    }
    ...
    void makeClusters() {
        this.clusterMap = new HashMap<Short, Cluster>();
    ...
    public void printClusters() {
        ...
        Iterator<Short> keys = this.clusterMap.keySet().iterator();
        logger.info("the " + this.clusterMap.size() +
                " clusters and their contents:");
        while (keys.hasNext()) {
            short grp = keys.next();
            Cluster cl = clusterMap.get(grp);
    ...
    public void rename() {
        ...
        for (Cluster cluster : clusters) {
            ...
            this.clusterMap.put(nn, clusters[ima]);
    ...
    boolean split() {
        ...
        for (Cluster cluster : clusters) {
            ...
            if (split) {
                ...
                part.fm = right;
                this.clusterMap.put(nr, part);
                short nl = this.next_cluster;
                this.next_cluster++;
                part = new Cluster(nl, cluster.history + "l");
                part.fm = left;
                this.clusterMap.put(nl, part);
                this.clusterMap.remove(cluster.group);
                this.logger.info
